...
One row per Warp tab, named after its task, with a live status. The menu-bar icon is a **crisp terminal tile** — teal while agents are working (Warp's colour), and **green / amber when a tab finishes or needs your input** — click it to jump straight to the **exact** tab the agent is in.
